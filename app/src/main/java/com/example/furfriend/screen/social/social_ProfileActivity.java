...
public class social_ProfileActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.d(TAG, "Viewing profile for userId: " + profileUserId + ", isOwnProfile: " + isViewingOwnProfile);
        ...
        Log.d(TAG, "RecyclerView setup complete, adapter: " + (postAdapter != null) +
                ", layoutManager: " + (recyclerProfileContent.getLayoutManager() != null));
                ...
                .addOnSuccessListener(document -> {
                    if (document.exists()) {
                        ...
                        if (avatarUrl != null && !avatarUrl.isEmpty()) {
                            Glide.with(this)
                                    .load(avatarUrl)
                                    .placeholder(R.drawable.social_profile_profile)
                                    .error(R.drawable.social_profile_profile)
                                    .into(ivAvatar);
                        }
                    } else {
                        Log.e(TAG, "User document does not exist for userId: " + profileUserId);
                    }
                })
    ...
    private void loadPosts() {
        ...
        Log.d(TAG, "Loading posts for userId: " + profileUserId);
        ...
        postsListener = db.collection("posts")
                ...
                .addSnapshotListener((querySnapshot, error) -> {
                    ...
                    if (querySnapshot == null) {
                        Log.d(TAG, "No posts snapshot received");
                        return;
                    }
                    ...
                    if (userIds.isEmpty()) {
                        Log.d(TAG, "No userIds to fetch after snapshot");
                        postAdapter.notifyDataSetChanged();
                        return;
                    }
                    ...
                    fetchUserNames(userIds, userNames -> {
                        ...
                        for (DocumentSnapshot document : addedDocs) {
                            ...
                            if (userId == null) {
                                Log.e(TAG, "Skipping post " + postId + ": userId is null");
                                continue;
                            }

                            String userName = userNames.getOrDefault(userId, "Unknown");
                            Log.d(TAG, "Creating post " + postId + " for user: " + userName);
                            ...
                            Log.d(TAG, "Notified adapter: insertPosition=" + insertPosition);
                        }
                        ...
                        for (DocumentSnapshot document : modifiedDocs) {
                            ...
                            if (userId == null) {
                                Log.e(TAG, "Skipping modified post " + postId + ": userId is null");
                                continue;
                            }

                            String userName = userNames.getOrDefault(userId, "Unknown");
                            Log.d(TAG, "Updating post " + postId + " for user: " + userName);
                            ...
                            for (int i = 0; i < contentList.size(); i++) {
                                ...
                                if (post.getPostId().equals(postId)) {
                                    ...
                                    Log.d(TAG, "Notified adapter: changedPosition=" + i);
                                    break;
                                }
                            }
                        }

                        Log.d(TAG, "Updated contentList size: " + contentList.size());
                    });
                });
    }
    ...
    private void loadLikes() {
        ...
        contentList.clear();
        postAdapter.notifyDataSetChanged();
        Log.d(TAG, "Loading liked posts for userId: " + profileUserId);
        ...
        likesListener = db.collection("posts")
                ...
                .addSnapshotListener((querySnapshot, error) -> {
                    if (error != null) {
                        Log.e(TAG, "Error listening to liked posts", error);
                        return;
                    }

                    if (querySnapshot == null) {
                        Log.d(TAG, "No liked posts snapshot received");
                        return;
                    }
                    ...
                    fetchUserNames(userIds, userNames -> {
                        ...
                        for (DocumentSnapshot document : addedDocs) {
                            ...
                            if (userId == null) {
                                Log.e(TAG, "Skipping liked post " + postId + ": userId is null");
                                continue;
                            }

                            String userName = userNames.getOrDefault(userId, "Unknown");
                            Log.d(TAG, "Creating liked post " + postId + " for user: " + userName);
                        ...
                        for (DocumentSnapshot document : modifiedDocs) {
                            ...
                            if (userId == null) {
                                Log.e(TAG, "Skipping modified liked post " + postId + ": userId is null");
                                continue;
                            }

                            String userName = userNames.getOrDefault(userId, "Unknown");
                            Log.d(TAG, "Updating liked post " + postId + " for user: " + userName);
    ...
    private void fetchUserNames(Set<String> userIds, OnUserNamesFetchedListener listener) {
        ...
        for (String userId : userIds) {
            db.collection("users").document(userId).get()
                    ...
                    .addOnFailureListener(e -> {
                        Log.e(TAG, "Error fetching username for userId: " + userId, e);
